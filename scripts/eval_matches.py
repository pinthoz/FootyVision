...
def main() -> None:
    ...
    print(
        f"{len(frame)} of {len(matches)} matches usable · "
        f"train {len(train)} · test {len(test)}\n"
    )

    def show(name: str, proba: np.ndarray, pred: np.ndarray) -> None:
        print(f"  {name:36s} acc {accuracy_score(y_test, pred):.3f}   "
...
def _significance(y_test: np.ndarray, pred: np.ndarray) -> None:
    ...
    print(f"  model right where the baseline is wrong: {model_only}, "
          f"and wrong where it is right: {baseline_only}")
    print(f"  McNemar chi2 {chi2:.1f}, p = {p:.1e}"
          f"   ({'significant' if p < 0.01 else 'not significant'})")
    print(f"  accuracy gain {np.mean(gaps):+.3f}, 95% CI [{low:+.3f}, {high:+.3f}]\n")


def _by_competition(test: pd.DataFrame, pred: np.ndarray, y_test: np.ndarray,
                    competitions: dict[int, str]) -> None:
    ...
    for competition_id, group in scored.groupby("competition_id"):
        ...
        print(f"    {competitions.get(competition_id, '?'):26s} n={len(group):4d}  "
...
def _ratings(matches: pd.DataFrame, teams: dict[int, str]) -> None:
    ...
    print(f"    expected goals {home_rate:.2f} v {away_rate:.2f}  ->  "
